...
public:
    ...
    void send_goal(int target_number, double period)
    {
        ...
        options.goal_response_callback = 
            std::bind(&CountUntilClientNode::goal_responce_callback, this, _1);
...
private:
    ...
    {
        auto status = result.code; // Checking the state of the result
    ...
    void goal_responce_callback(const CountUntilGoalHandle::SharedPtr &goal_handle)
    {
        if (!goal_handle){
